...
int MCRectShapeImpl::interpenetrationDepth(const MCVector2d<MCFloat> & p1, const MCVector2d<MCFloat> & p2) const
{
  MCEdge<MCFloat> edge(edgeForPoint(p2));
  return MCMathUtil::distanceFromVector(p1 - edge.origin, edge.edge);
}

MCRectShape::MCRectShape(MCObject * pParent, MCShapeView * pView, MCFloat width, MCFloat height) :
...
int MCRectShape::interpenetrationDepth(const MCVector2d<MCFloat> & p1, const MCVector2d<MCFloat> & p2) const
{
  return m_pImpl->interpenetrationDepth(p1, p2);
}

MCVector2d<MCFloat> MCRectShapeImpl::contactNormal(const MCVector2d<MCFloat> & p) const
{
  // Get the "nearest" edge for p and build a normal vector from it
  static const MCVector3d<MCFloat> DOWN(0, 0, -1);
  return MCVector2d<MCFloat>((MCVector3d<MCFloat>(edgeForPoint(p).edge) * DOWN)).normalizedFast();
}

MCVector2d<MCFloat> MCRectShape::contactNormal(const MCVector2d<MCFloat> & p) const
{
...
{
  m_pImpl->m_obbox = MCOBBox<MCFloat>(width / 2, height / 2, location());
  m_pImpl->m_obbox.rotate(angle());
}
